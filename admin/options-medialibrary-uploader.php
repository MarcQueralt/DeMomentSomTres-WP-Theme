...
if ( ! function_exists( 'optionsframework_mlu_init' ) ) {
	function optionsframework_mlu_init () {
		register_post_type( 'optionsframework', array(
			'labels' => array(
				'name' => __( 'Options Framework Internal Container','demomentsomtres' ),
			),
			'public' => true,
			'show_ui' => false,
...
if ( ! function_exists( 'optionsframework_medialibrary_uploader' ) ) {
	...
	function optionsframework_medialibrary_uploader( $_id, $_value, $_mode = 'full', $_desc = '', $_postid = 0, $_name = '') {
		...
		$output .= '<input id="upload_' . $id . '" class="upload_button button" type="button" value="' . __( 'Upload','demomentsomtres' ) . '" rel="' . $int . '" />' . "\n";
		...
		if ( $value != '' ) { 
			...
			if ( $image ) {
				...
				$title = __( 'View File', 'demomentsomtres' );
...
if ( ! function_exists( 'optionsframework_mlu_modify_tabs' ) ) {
	...
	function optionsframework_mlu_modify_tabs ( $tabs ) {
		$tabs['gallery'] = str_replace( __( 'Gallery', 'demomentsomtres' ), __( 'Previously Uploaded', 'demomentsomtres' ), $tabs['gallery'] );
		return $tabs;
	}
}